...
function PortalNoticeGate({ children }) {
  ...
  const fetchPendingNotice = async ({ allowClear = false } = {}) => {
    try {
      setLoading(true);
      setError("");

      const response = await API.get("/portal-notices/pending");
      const pendingNotice = response.data?.notice || null;
      ...
      setNotice((currentNotice) => {
        ...
        return allowClear && !currentNotice ? null : currentNotice;
      });
    } catch (requestError) {
      setError(
  ...
  useEffect(() => {
    fetchPendingNotice({ allowClear: true });
  }, []);
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      setNotice(null);
      await fetchPendingNotice({ allowClear: true });
    } catch (requestError) {
      setError(
        requestError.response?.data?.message ||
          requestError.response?.data?.error ||
  ...
  return (
    <>
      ...
      {(loading || notice || error) && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/65 px-5 py-8 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-lg bg-secondary p-6 shadow-2xl md:p-8">
            {loading ? (
              ...
              <>
                ...
                >
                  ...
                  {acknowledging
                    ? "Saving..."
                    : "I have read this announcement, continue"}
                </button>
              </>
            )}
          </div>
